...
`Sandbox`作为独立业务项目，通过引擎提供的`CreateApplication()`回调接入：

```cpp
// Sandbox项目核心代码
class Sandbox : public Hazel::Application {
public:
    Sandbox() { PushLayer(new ExampleLayer()); } // 加载游戏层
};
...
## 2. Hazel.h 扩充：引擎统一入口头文件

之前业务层需要逐个引入分散的头文件，现在我扩充了`Hazel.h`，让它成为引擎的 “一站式入口”：
...
现在业务层只需要`#include <Hazel.h>`，就能调用所有引擎功能，开发效率大幅提升。
